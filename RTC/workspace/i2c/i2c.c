...
#define SLAVE_ADDR  0x68 // 0b01101000
...
#define CMD_TYPE_0_MASTER      0x07
#define CMD_TYPE_1_MASTER      0x00
...
uint8_t MasterType1 [TYPE_1_LENGTH] = {0x00};
uint8_t MasterType0 [TYPE_0_LENGTH] = {0x13};
...
int main(void) {
    ...
    uint8_t second_off = 0xa5; // 0b1001_0101 // stop, 15 seconds
    uint8_t minute = 0x07; // 0b0000_0111 // 7 minutes
    uint8_t hour = 0x16; // 0b0001_0110 // 24 hour clock, 16 hours
    uint8_t weekday = 0x05; // 0b0000_0101 // thursday
    uint8_t monthday = 0x27; // 0b0010_0111 // day 27
    uint8_t month = 0x02; // 0b0000_0010 // month 2
    uint8_t year = 0x20; // 0b0010_0000 // year 2020
    uint8_t second_on = 0x45; // 0b0001_0101 // start, 15 seconds
    uint8_t data_to_write [8] = {second_off, minute, hour, weekday, monthday, month, year};

    printf("Initializing...\n");
    I2C_Master_WriteReg(SLAVE_ADDR, 0x00, data_to_write, 8);
    printf("Starting...\n");
    I2C_Master_WriteReg(SLAVE_ADDR, 0x00, &second_on, 1);

    uint16_t i, j;
    for (i = 0; i < 10000; i++);
    printf("Reading...\n");
    I2C_Master_ReadReg(SLAVE_ADDR, 0x00, 8);
    for (i = 0; i < 10000; i++);
    for (i = 0; i < 7; i++)
    {
      printf("Data at addr %d: %x\n", i, ReceiveBuffer[i]);
    }
    printf("Waiting...\n");
    for (j = 0; j < 1000; j++)
    {
